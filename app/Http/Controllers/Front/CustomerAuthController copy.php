        ...
        if ($request->has('registration_code')) {
            $epin = EPinTransfer::where('e_pin', $request->registration_code)
                ...
                if ($epin) {
                    ...
                    if ($user) {
                            // Redis logic to handle payment to the next "star" user
                            $starUsers = User::where(['is_active'=>1, 'is_green'=>1, 'status'=>'Active', 'package_id'=>'1'])
                                            ->orderBy('activated_date')
                                            ->pluck('id')
                                            ->toArray();
        
                            // Get the last user who received payment
                            $lastUserId = Redis::get('last_star_user');
        
                            // Find the next user to receive payment
                            $nextUserIndex = array_search($lastUserId, $starUsers);
                            $nextUserIndex = ($nextUserIndex === false || $nextUserIndex === count($starUsers) - 1) ? 0 : $nextUserIndex + 1;
                            $nextUserId = $starUsers[$nextUserIndex];
        
                            // Update the Redis key with the new last user ID
                            Redis::set('last_star_user', $nextUserId);
        
                            // // Process the payment (replace with actual payment logic)
                            // $receiver = User::find($nextUserId);
                            // $receiver->help += 300;
                            // $receiver->save();
